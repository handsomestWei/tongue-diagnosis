...
async def infer(
    ...
    if image_id is not None:
        ...
        kind = row.image_kind
        if image_kind is not None and image_kind != kind:
            raise HTTPException(
                status_code=400,
                detail=f"表单 image_kind 与库中记录不一致（库: {kind}, 表单: {image_kind}）",
            )
        path = repo.abs_storage_path(row)
        if not path.is_file():
            raise HTTPException(status_code=404, detail=f"存储文件缺失: {path}")
        image_bgr = _read_bgr_from_upload_bytes(path.read_bytes())
        sam_path = path if kind == "full_face_selfie" else None
    else:
        ...
        if image_kind is None or image_kind not in allowed:
            raise HTTPException(
                status_code=400,
                detail=f"上传文件时须指定 image_kind，允许 {sorted(allowed)}",
            )
        kind = image_kind
        raw = await file.read()
        if not raw:
        ...
        sam_path = None  # bytes-only upload：SAM 子进程用临时文件
