...
- **上传接口路径**：图片上传为 **`POST /api/v1/images/upload`**（multipart：`file` + 必选表单字段 **`image_kind`** ∈ `full_face_selfie` | `tongue_closeup`）。标注为 **`PATCH /api/v1/images/{id}/labels`**。更新元数据 / **立即重算规范舌图**：**`PATCH /api/v1/images/{id}`** 可设 `image_kind`、`regenerate_derived=true`（会写 `derived_tongue_path`；全脸需 TongueSAM）。**viewer 角色**对该类写操作会 **403**（由后端强制）。
